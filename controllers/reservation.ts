...
export const getCustomerReservations = (req: Request, res: Response, db: Knex) => {
    db.transaction(trx => {
        trx.select('*').from('reservations').where('customerId', req.body.id)
            .then(data => {
...
export const getRestaurantAcceptedReservations = (req: Request, res: Response, db: Knex) => {
    ...
    db.transaction(trx => {
        trx.select('*').from('reservations').where('restaurantId', req.body.id).andWhere('status', 'approved')
            .then(data => {
                trx.commit();
                res.status(200).json(data);
            })
...
export const getRestaurantWaitingReservations = (req: Request, res: Response, db: Knex) => {
    db.transaction(trx => {
      trx.select('*').from('reservations').where('restaurantId', req.body.id).andWhere('status', 'waiting')
          .then(data => {
              trx.commit();
              res.status(200).json(data);
          })
